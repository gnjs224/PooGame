...
class GameViewController: UIViewController {
    ...
    let avoiderImageView = UIImageView(frame: CGRect(x: UIScreen.main.bounds.width / 2 - 20, y: UIScreen.main.bounds.height - 200 , width: 40, height: 40))
    ...
    let avoiderImageChangeSpeed: UInt32 = 50000
    let pooImageChangeSpeed: UInt32 = 1000
    let avoiderMovingDistance: Double = 0.03
    ...
    func progress(){
        var d: CGFloat = 0
        //속도
        if moveState == .rightRun {
            d = avoiderMovingDistance
        } else if moveState == .leftRun {
    ...
    func gameOver() {
        self.moveState = self.moveState.getDirection() == "left" ? MoveState.leftDie : MoveState.rightDie
        self.timer?.invalidate()
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
            if self.navigationController?.viewControllers.last !== nil {
                
                if let coordinator = self.coordinator as? GameCoordinator {
                    coordinator.score = self.score
    ...
    func stopAvoider(_ sender: UIButton){
        if moveState.isRunning() {
            moveState = moveState.getDirection() == "left" ? .leftStop : .rightStop
        }
    }
